...
in [on-demand](/devhub/compute-resources/functions/#on-demand-execution) or [persistent](/devhub/compute-resources/functions/#persistent-execution) execution modes. They are typically used to store
mutable data such as databases and persist data. Currently, persistent volumes are only stored on
the [Compute Resource Node](/devhub/compute-resources/standard-instances/) (CRN) that is executing the program. Automatic backups and restoration
